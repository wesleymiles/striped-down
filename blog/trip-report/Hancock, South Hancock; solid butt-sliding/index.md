---
layout: blog.liquid
title: Hancock, South Hancock; solid butt-sliding
description: A January hike and camp where the sledding gods shone down on us
tags: 
  - post
  - trip-report
date: 2025-10-15
trips:
  dateHiked: 2017-01-28
  peaks:
    - name: Mt. Hancock
      elevation: 4403
    - name: South Hancock
      elevation: 4319
  trail: Hancock Notch 
  distance: 9.8 miles
  # temp: 0F° on top
location: 
  town: Lincoln
  state: New Hampshire
---

This trip was 8 years ago, but I did find a 4 sentance jounal entry and emails back and forth planning the trip to help build the story in my memory of this one. 

<hr>

A group of us pulled into the trailhead off the Kancamagus, after the hairpin turn in late January. We started the hike early and planned to camp the night near the base. I don't know exactly where we camped, but according to the photo location data, I think it was about 2 miles in, before the trail started to ascend. 

According to the trip reports, the previous days had rain and sleet and hikers dealt with postholing galore. Lucky for us, the cold was back, so we prepared for ice by bringing spikes rather than snowshoes.

A note I wrote about the most challenging part of this loop: 

<blockquote>

I was prepared and ready for the brutal .7 mile, 1000 foot climb.

</blockquote>

Also, 

<blockquote>

Most fun I've had sledding, literally, ever.

</blockquote>

Since then, I may knock it down to the three sleds of my life. 
 
<hr>


{% image "img/IMG_9875 10.44.45 PM.JPG", "Hiking photo from 2017-01-28", "We had full packs until the campsite" %}


{% image "img/IMG_9876 10.44.45 PM.JPG", "Hiking photo from 2017-01-28", "Selfie sporting a new wool cap" %}

I wore a thin wool shirt on the way up until clouds and wind came and I had to fully suit-up. 


<div class="grid bleed ">
{% image "img/IMG_9891 10.44.45 PM.JPG", "Hiking photo from 2017-01-28", "No more full packs, only sled in hand – the only way to winter hike." %}
</div>

{% image "img/IMG_9898 10.44.45 PM.JPG", "Hiking photo from 2017-01-28", "Meandering up; a stark contrast to the muted forest " %}


{% image "img/IMG_9901 10.44.45 PM.JPG", "Hiking photo from 2017-01-28", "Spotting the ski runs of Bretton Woods through crusty branches" %}

<div class="grid bleed cols-2">
{% image "img/IMG_9905 10.44.45 PM.JPG", "Hiking photo from 2017-01-28", "No wings" %}
{% image "img/IMG_9906 10.44.45 PM.JPG", "Hiking photo from 2017-01-28", "The white mountain's famously popular gray jay" %}
</div>


{% image "img/IMG_9908.JPG", "Hiking photo from 2017-01-28", "From the summit of Mt. Hancock" %}


Apparently some bushwhackers take the [avalanched lanslide](https://dailey7779.blogspot.com/2010/09/arrow-slide.html) as a route up. When there's no snow, the white "v" here is exposed rock.  

{% image "img/arrow-slide.JPG", "Hiking photo from 2017-01-28", "The arrow slide" %}


<hr>

On this hike, I was practicing a new skill: identifying trees from their bark. Winter is the time to do this because you were freed from the crutches of getting hints from leaves. To help me learn, I was building visual catalog of different species. Here are a few additions. I unfortunetly I lost that catalog to the digital winds. 

<div class="grid bleed cols-2">

  {% image "img/white-birch.JPG", "Hiking photo from 2017-01-28", "White birch (one of the easiest to ID)" %}
  {% image "img/balsam-fir.JPG", "Hiking photo from 2017-01-28", "Balsam fir" %}
</div>



{% image "img/IMG_9910 10.44.45 PM.JPG", "Hiking photo from 2017-01-28", "Preparing for a butt-slide. Keep those microspikes up! 🤙🏻" %}


{% image "img/MAP-hancock-south-hancock.jpg", "Hiking photo from 2017-01-28", "Simplified map of the loop, from <a href='https://4000footers.com/hancock.shtml'>4000footers.com</a>." %}


Weariness hit me an hour from the car.

It was possibly delayed by that pint at the summit.



