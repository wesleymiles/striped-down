...
I wanted all the good parts of google photos, the UI/UX, drag-and-drop, and smart search, without the google. I wanted data sovereignty, as the privacy crowd calls it, for my bits to live on my hardware not on big techs. My metric for success was not seeing recurring subscription fees for my photo library on my credit card. 
...
I dug out the 2012 MacBook that was sitting among books and I wiped it clean. Then downloaded Ubuntu Server and flashed it to a USB drive using Balena Etcher. I moved the USB to the old machine and installed it there. This was my first time installing a server operating system. I often use the terminal when editing my 11ty site, but working like this still felt foreign. Text-based installers, no mouse, navigating with arrow keys and tab through config screens.

Getting wifi working took longer. The laptop's wifi chip needed drivers that Ubuntu didn't include by default. I spent an hour editing network config files, installing driver packages, and restarting network services until finally... SSH access from my main computer. I relished this <em>"hello world"</em> moment. 

I decided on Immich for an open source photo app. I installed Docker, got Immich running, formatted the external drives, and started importing photos. Whoa, I'd actually pulled this off.

Then, no. 

The external drives were dropping power under sustained write load of generating thumbnails for thousands of photos. The exFAT filesystem would detect errors and switch to read-only mode. Immich would crash. I'd remount the drives, restart everything, and the cycle would repeat. After hours of trying different USB ports, different filesystems, and different configs, I accepted that the old USB ports couldn't power modern 2TB drives during intensive operations. 

I wish AI could have foreseen the macbook hardware glitches (anger emoji).

## Attempt 2: purpose-built hardware (4 hours)

So my fears came to pass. I was troubleshooting this thing to dry eyeballs during my vacation time. I can’t say it wasn’t somewhat enjoyable though. I was chasing the programmer's dragon.

I researched and ordered a new computer, landing on a GEEKOM mini PC with proper USB power delivery. It was more bang for my buck than a Pi. I planned to reuse the previous software (Ubuntu Server, Immich) and hardware (2x 2TB external drives) from attempt 1. 
...
During this time of copy and pasting commands from claude into terminal, I had this sense that this was the future. A constant open conversation with a generative AI tool. But, I wondered how much I was learning. If we consider memorizing commands learning, I wasn’t doing that. If understanding the steps in the process is learning, I was partially doing that. I had claude explain what each line meant. My memory of that time is a blur of rage-copy-pasting. I don't feel I retained much. Maybe writing a blog about an AI experience is required for the learning to settle, because this seems to be working. 

I was uneasy about my reliance on it. But then I suppose that’s not new. It’s like tightly relying on the manual in the middle of repairing your first engine.
